...
\begin{abstract}
%\boldmath
We try to learn how the task image classification could be solved. To work on such a task we use the cifar10 dataset.  In a detailed problem description in we pointing out all of the essential points to understand the given task and dataset. To have a broader understanding of what does research contributes to this task of image recognition, we gave a summary of the latest publications and developments around the image classification field. To solve the job with our implementation, we decided to use a VGG16 model architecture. So, we show how our architecture looks in detail and explain all its layers. In the end, we analyze the results of our implementation while using the results itself and while using the results of similar applications as a baseline. 
\end{abstract}
...
\noindent As mentioned in our \hyperref[sec:problemDescription]{"Problem Desciption"}, used as a training example for an image recognition task, Cifar10 is quite popular. In the online paper archive of the Cornell Univerity, we found 336 publications as we searched for the term "cifar10" \footnote{\href{https://paperswithcode.com/sota/image-classification-on-cifar-10}{https://paperswithcode.com/sota/image-classification-on-cifar-10}}. Besides the publications using cifar10, you could notice a couple of rankings. The rankings are mostly referring to the overall test accuracy or the total loss \cite{echersly}. For simplification, we are relating to the classification of web page "paperswithcode.com" \footnote{\href{http://arxiv.org/}{http://arxiv.org/}}. We can't focus on each of those papers, publications, architectures and rankings which are using the cifar10 as a baseline. In the following, we gave a closer look at the most important publications. Important means,  publications which provide a model which is under the most successful architectures. More precisely, architectures for solving the image classification task with the cifar10 job.
\subsection{GPipe}
While in smaller experiments, the training processes of a model is quite small, the time for computing the weights is increasing with an increase in size and complexity\cite{Yao2017ComplexityVP}.\\
Researchers from Google implemented a machine learning library which offers parallelization for the training process of a machine learning model. GPipe uses mini-batch gradient descent and pipeline parallelism. The goal is that \(k\) accelerators can calculate a defined part of the model \(k\). If we could split our model into \(k\)  parts, we could scale the training process even higher \cite{GPipe}. That leads to an outstanding performance in machine learning tasks as image recognition\cite{perfGPipe}. While using real code examples the research team could verify their work. They reached an accuracy in an image classification task using the dataset cifar10 from 99\% \cite{GPipe}. 
\subsection{EfficientNet}
While we could use the GPipe for parallelisation of the training task while we had a complicated and colossal model, EfficientNet is trying to get the same results in a more efficient process\cite{EfficientNet} . So instead of using more resources, they providing a super-efficient method to get better or similar results while using fewer resources with neural networks which are already used.\cite{EfficientNet} For example, they could increase the accuracy from the ResNet up to 10\% almost without using extra resources.
\subsection{PyramidNet+ShakeDrop}
\subsection{WRN-28-10 + SWA}
%
...
\noindent Data augmentation describes the method of creating new data points by transforming the original data. If we look at images, this can be done for example by resizing, rotating, shifting, flipping and more.
...
\noindent The goal of normalization is to make every datapoint have the same scale so each feature is equally important. There are many possible ways to scale data e.g. MinMax-Scaling or Z-Score and all of them follow certain purposes. 
...
We decided to use the VGG16 model architecture. In 2014 an implementation of the architecture could win ILSVR(Imagenet) competition. Even if in the overall cifar10 ranking are more powerfull architectures and architecture combinations, we decided to use this classic example.  The complexity is small in comparison to  other designs which are provided in the ranking. Note that - as  mentioned in the \hyperref[sec:problemDescription]{"Problem Desciption"}, the goal of our project was not to get an outstanding result of the accuracy. The main goal was more to understand the principals behind deep learning and to get a feeling of what the state of the art (in image recognition) is. So, because the VGG16 is known as the best visualizable model architecture, we thought it would be the best choice to accomplish our goal. The model architecture is also known for getting rid of a vast amount of hyper-parameters. Instead, it is focusing on tiny filter size and a small strate. The technical details \hyperref[sec:techDetails]{"Problem Desciption"}, of the different layers are providing more details about its implementaiton. 
\subsubsection{Convolutional Layers}
\noindent The convolutional Layer is for extracting features from the input matrix into a feature map. For this procedure, we use matrix multiplication and a filter (feature detector, kernel) \cite{Goodfellow-et-al-2016}. In picture 9 you can see a calculation example of how convolution is applied to a matrix. We are iterating with the filter over the matrix, calculating the scalar product and writing the result into a feature map. An activation function like the rectified linear unit (Relu) normalizes the feature map after we did the convolution.  Normalization grantees a feature map which is not a linear transformation of its input value. If that would be the case we had just a linear problem what would be easily solved but with a wrong result. In other words, the input values get just multiplied by coefficients. Note that there are plenty of reasons for activation functions in a neuronal network. We are giving a closer look at this topic at our section \hyperref[sec:activationFunctions]{"Activation Functions"}. In Figure 8 you can see how our pictures had changed after applying the first convolutional layer, before and after doing normalization.  \\
